...
const RoomDetails = ({ roomNumber, room, updateRoom, deleteRoom, canDelete, adherent, remise }) => {
  ...
  return (
    <Box sx={{ mb: 2, bgcolor: 'background.paper', p: 2, borderRadius: 'borderRadius', display: 'flex', flexDirection: 'column', gap: 1 }}>
      ...
      <IconButton onClick={() => decrementAdults(room)} disabled={room.nbr_adults <= 1} color="primary">
...
<TextField
  ...
  sx={{ width: '60px', mx: 1 }}
/>
<IconButton onClick={() => incrementAdults(room)} disabled={room.nbr_adults >= 4} color="primary">
  <AddCircleOutlineIcon />
</IconButton>

<IconButton onClick={() => decrementChildren(room)} disabled={room.nbr_enfants <= 0} color="primary">
  <RemoveCircleOutlineIcon />
</IconButton>
<TextField
  size="small"
  ...
  sx={{ width: '60px', mx: 1 }}
/>
<IconButton onClick={() => incrementChildren(room)} disabled={room.nbr_enfants >= 3} color="primary">
  <AddCircleOutlineIcon />
</IconButton>
...
const ModifyReservation = ({
  ...
  const handlePersonChange = (newNombre, newNombreEnfants) => {
    ...
    const newTotalPrice = (newNombre * basePricePerAdult) + (newNombreEnfants * pricePerChild);
    setPrixTotal(newTotalPrice * (adherent ? 1 - remise : 1));
  };
  
  const calculateRoomPrice = (adults, children, basePrice,prixEnfants) => {
    let priceIncrease =
      (adults - 1) * basePrice  + children * prixEnfants;
  ...
  const [nombreEnfants, setNombreEnfants] = useState(reservationData.nbr_enfants || 0);
...
/>
    {reservationData.offre.enfants_autorises && (reservationData.typeR === 'voyage' || reservationData.typeR === 'activite') && (
      <Box sx={{ mt: 2 }}>
