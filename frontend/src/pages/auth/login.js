...
function Login() {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      await axios.post('http://54.87.28.4/forgot-password', { email });
      MySwal.fire(
        'Succès',
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('http://54.87.28.4/login', {
        email,
        motDePasse,
      });
      ...
      localStorage.setItem(
        'userId',
        JSON.stringify(user.id_utilisateur.toString())
            ...
            willClose: () => {
              if (document.getElementById('resendEmailBtn')) {
                document
                  ...
                  .removeEventListener('click', handleResendEmail);
              }
            },
          });
        }
  ...
  const handleResendEmail = async () => {
    ...
    try {
      await axios.post('http://54.87.28.4/resendEmail', { email });
      MySwal.fire(
        'Email envoyé',
