...
const MyReservations = () => {
    ...
    useEffect(() => {
      ...
      const fetchReponseReservations = async () => {
          try {
              const response = await axios.get('http://localhost:5000/reservationsRe', {
                  headers: { Authorization: `Bearer ${token}` },
              });
                        ...
                        <Card raised sx={{ height: 600, overflowY: 'auto' }}>
                            <CardContent>
                                {filteredDemandeReservations.map((reservation) => (
                                   <Card key={reservation.id_reservation} variant="outlined" sx={{ height: 150, mb: 2, display: 'flex', backgroundColor: getCardBackgroundColor(reservation.etat), cursor: 'pointer' }} onClick={() => handleOpenDialog(reservation)}>
                                   <Box sx={{ position: 'relative', width: 100, height: 100 }}>
                                       ...
                                       />
                                       <Typography variant="body2" sx={{ position: 'absolute', bottom: -30, width: '100%', textAlign: 'center', color: 'black', fontSize: '0.8rem' }}>
                                           {`${reservation.employe.utilisateur.nom} ${reservation.employe.utilisateur.prenom}`}
                                       </Typography>
                                       <Typography variant="body2" sx={{ position: 'absolute', bottom: -45, width: '100%', textAlign: 'center', color: 'black', fontSize: '0.8rem' }}>
                                           {reservation.employe.adherant ? 'Adhérent' : 'Non Adhérent'}
                                       </Typography>
                                   </Box>
                                            ...
                                            {reservation.etat === 'reparation' && (
                                                <Box display="flex" justifyContent="flex-end">
                                                    <Button size="small" variant="contained"
                                                        ...
                                                        onClick={(e) => handleAccept(reservation.id_reservation, e)}>
                                                        Accepter
                                                    </Button>
                                                    <Button size="small" variant="contained" 
                                                        sx={{ backgroundColor: '#C50F10', '&:hover': { backgroundColor: '#D0C170' }, mr: 1 }}
                                                        onClick={(e) => handleRefuse(reservation.id_reservation, e)}>
                                                        Refuser
                                                    </Button>
                                                </Box>
                                            )}
