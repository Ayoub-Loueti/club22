...
const DeuxDash = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:5000/userstats', {
                    headers: { Authorization: `Bearer ${token}` },
                  });
                const { clientsPercentage, employesPercentage } = response.data;
                setData([
                    { id: 'Clients', label: 'Clients', value: parseFloat(clientsPercentage) },
                    { id: 'Employés', label: 'Employés', value: parseFloat(employesPercentage) },
                ]);
            } catch (error) {
